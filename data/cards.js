...
function validateBingo(cardId, calledNumbers) {
    ...
    const isMarked = (r, c) => {
        ...
        return num === 0 || calledSet.has(num);
    };
    for (let r = 0; r < 5; r++) {
        let rowWin = true;
        for (let c = 0; c < 5; c++) if (!isMarked(r, c)) { rowWin = false; break; }
        if (rowWin) return true;
    }
    for (let c = 0; c < 5; c++) {
        let colWin = true;
        for (let r = 0; r < 5; r++) if (!isMarked(r, c)) { colWin = false; break; }
...
if (typeof module !== 'undefined') module.exports = { BINGO_CARDS, validateBingo };